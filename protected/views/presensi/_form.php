	...
	<?php echo $form->datepickerRow($model,'tanggal',array('class'=>'span5', 'value'=>date('Y-m-d'),'options'=>array('dateFormat'=> 'yy-mm-dd'),));?>
	<?php 
		$bulan=date('m');
		if($bulan<7){
			$smt=2;
...
<?php 
	if(Yii::app()->user->checkAccess('Admin')){
		$this->widget('bootstrap.widgets.TbTabs', array(
				...
				'tabs' => array(
						array('label' => 'WALI', 'active'=>true, 'visible'=>Yii::app()->user->checkAccess('Wali'), 'icon'=>'icon-list-alt', 'content' => $this->renderPartial('_dialogwali', array('model'=>$model, 'model2'=>$model2), true)),
						array('label' => 'GURU PIKET', 'visible'=>Yii::app()->user->checkAccess('Piket'), 'icon'=>'icon-list-alt', 'content' => $this->renderPartial('_dialogpiket', array('model'=>$model, 'model2'=>$model2), true)),
				),
		));
	}else{
		$this->widget('bootstrap.widgets.TbTabs', array(
				...
				'tabs' => array(
						array('label' => 'WALI', 'active'=>true, 'visible'=>Yii::app()->user->checkAccess('Wali'), 'icon'=>'icon-list-alt', 'content' => $this->renderPartial('_dialogwali', array('model'=>$model, 'model2'=>$model2), true)),
						array('label' => 'GURU PIKET', 'active'=>true, 'visible'=>Yii::app()->user->checkAccess('Piket'), 'icon'=>'icon-list-alt', 'content' => $this->renderPartial('_dialogpiket', array('model'=>$model, 'model2'=>$model2), true)),
				),
		));
	}
?>
